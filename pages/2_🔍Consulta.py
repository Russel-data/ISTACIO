...
if not st.session_state.logged_in:
    ...
    for campo in campos_para_datas:
        if campo in dados_formatados.columns:
            dados_formatados[campo] = pd.to_datetime(dados_formatados[campo], errors='coerce').apply(lambda x: x.strftime('%d/%m/%Y') if pd.notnull(x) else '')
        ...
        if not dados_filtrados.empty:
            ...
            for campo in campos_para_datas:
                if campo in dados_filtrados.columns:
                    dados_filtrados[campo] = pd.to_datetime(dados_filtrados[campo], errors='coerce').apply(lambda x: x.strftime('%d/%m/%Y') if pd.notnull(x) else '')
